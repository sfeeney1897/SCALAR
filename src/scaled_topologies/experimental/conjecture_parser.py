...
if __name__ == '__main__':
    # Parses the experimental/ (held-out "test" batch) stratified conjecture
    # output — produced by running conjecture_stratified.py against the
    # experimental test batch instead of run_known_random_p5_merged.csv.
    from utils import test_utils
    test_utils.set_dir(test_utils.get_path())
    df = parse_conjecture_file("../../../results/scaled_topologies/experimental/conjecture_output_test.txt")
    df.to_csv("../../../results/scaled_topologies/experimental/conjectures_parsed_test.csv", index=False)
